...
class StableDiffusionTest():
    ...
    def __init__(self, model_id,proj_path,device):
        ...
        self.pipe = AutoPipelineForText2Image.from_pretrained(model_id, torch_dtype=torch.float16, variant="fp16").to(device) 

        self.pipe.scheduler = LCMScheduler.from_config(self.pipe.scheduler.config)

        # load and fuse lcm lora
        self.pipe.load_lora_weights("/public_data/ma/models/lcm-lora-sdxl")
        self.pipe.fuse_lora()
...
if __name__ == '__main__':
    ...
    sdt = StableDiffusionTest(model_id,proj_path,device,Lora)
    ...
    while True:
        ...
        if raw_text == "stop":
            break
        negative_prompt = input("\nPlease negative_prompt >>> ")
        if not negative_prompt:
            negative_prompt="低分辨率、画框、镜子、宝丽来、黑暗环境"
